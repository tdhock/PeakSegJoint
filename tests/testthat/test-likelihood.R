...
for(sample.i in seq_along(profile.list)){
  ...
  results$status <- with(results, {
    ifelse(fit.loss == min(fit.loss), "best", "not")
  })
  best.peak <- subset(results, fit.loss == min(fit.loss))[1, ]
  results$chromStart.diff <- results$chromStart - best.peak$chromStart
  results$chromEnd.diff <- results$chromEnd - best.peak$chromEnd

  loss.list[[sample.id]] <- data.frame(sample.id, results)
  
  sample.peak.list[[sample.id]] <-
    data.frame(sample.id,
               y=-0.1 * (1:nrow(results)) * max(sample.counts$count),
               results)
}
...
loss$param.fac <- factor(loss$param)
...
ggplot()+
  ...
  geom_point(aes(param.fac, fit.loss, size=status),
             data=data.frame(fit.loss, what="loss"))+

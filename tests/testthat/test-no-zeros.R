...
test_that("Step1 C result agrees with R (no zero counts)", {
  profiles <- data.list$without.zero
  bin.factor <- 4L
...
test_that("Step2 C result agrees with R (no zero counts)", {
  profiles <- data.list$without.zero
  bin.factor <- 4L
  fit <- PeakSegJointHeuristicStep2(profiles, 4)
  ...
  for(peaks.str in names(best.peak.list)){
    loss.best <- best.indices.list[[peaks.str]]
    best.peak.df <- best.peak.list[[peaks.str]]
    samples.with.peaks <- paste(best.peak.df$sample.id)
